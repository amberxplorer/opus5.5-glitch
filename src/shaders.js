...
void main() {
  ...
  float line = p.y;
  float off = (hash(vec2(line, uFrame)) - 0.5) * 1.8 * uAmt;
  ...
  iq /= 8.0;
  Y += 0.06 * length(iq) * cos(3.14159 * (p.x + line + uFrame));
  Y += 0.11 * luma(px(x - 18.0, line));
  Y += (hash(p + uFrame * 13.0) - 0.5) * 0.09 * uAmt;
  Y += trk * (hash(p + uFrame * 3.0) - 0.4) * 0.8;
...
void main() {
  ...
  col *= vis * boost;
  if (uSnow > 0.0) col = mix(col, vec3(hash(floor(sp / max(1.0, uK)) + fract(uT * 7.13) * 431.0)), uSnow);
  col += (hash(sp + fract(uT * 3.7) * 311.0) - 0.5) * uGrain;
  col *= 1.0 - uFade;
